...
export default class ReviewCommentsComponent extends Component {
    ...
    sendMessage() {
        try {
            this.clientRef.sendMessage('/app/comment', JSON.stringify({
    ...
    render() {
        ...
        return (
            <div>
                <SockJsClient url={API_BASE_URL + 'comment-websocket'}
                              topics={['/topic/comment/' + this.state.reviewId]}
                        ...
                        <div key={index} className="commented-section bg-white mt-2 mb-2 rounded p-3 shadow">
                            ...
                            <div className="reply-section">
                                <div className="d-flex flex-row align-items-center voting-icons">
                                    <a className="text-primary mt-1" type="button" id={comment.authorName}
                                       onClick={this.reply}>Reply</a>
                                </div>
                            </div>
                        </div>
                    ) : (
